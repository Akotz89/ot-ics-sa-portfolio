...
| Gap-analysis prompts | The whiteboard turns known scenario gaps into customer-safe validation prompts and PoV dependencies. | `discovery-as-found`, `demo-thesis`, `architecture-design` | PASS |

Validation gates:

- `npm run audit:whiteboard:scenario` requires evidence references on every zone, entity, link, and step.
- `whiteboard/GAP_ANALYSIS.md` maps customer concerns to steps, scenario findings, discovery prompts, Dragos relevance, and evidence.
