...
const entityDefinitions: EntityModel[] = [
  ...
  { id: 'jump', kind: 'jump', label: 'Jump Host', subtitle: 'admin access, MFA/logs', zone: 'boundary', x: 550, y: 260, w: 172, h: 78, role: 'boundary' },
  ...
  { id: 'sensor-boundary', kind: 'sensor', label: 'Boundary Sensor', subtitle: 'passive SPAN/TAP', zone: 'monitoring', x: 88, y: 650, w: 220, h: 78, role: 'dragos' },
  { id: 'sensor-control', kind: 'sensor', label: 'Control Sensor', subtitle: 'passive SPAN/TAP', zone: 'monitoring', x: 348, y: 650, w: 220, h: 78, role: 'dragos' },
...
const linkDefinitions: LinkModel[] = [
  ...
  { id: 'firewall-sensor-boundary', source: 'firewall', target: 'sensor-boundary', kind: 'span', direction: 'passive', label: 'passive boundary SPAN/TAP', sourceAnchor: 'right', targetAnchor: 'top', sourcePort: 'right', targetPort: 'uplink', routeClass: 'span-feed', directionCue: 'passive', labelPolicy: 'first-visible', lanePreference: 'external', customerMeaning: 'passive north/south boundary traffic copy from OT-side firewall or core path' },
  { id: 'dist-sensor-control', source: 'dist', target: 'sensor-control', kind: 'span', direction: 'passive', label: 'passive control SPAN/TAP', sourceAnchor: 'left', targetAnchor: 'right', sourcePort: 'left', targetPort: 'span', routeClass: 'span-feed', directionCue: 'passive', labelPolicy: 'first-visible', lanePreference: 'external', customerMeaning: 'passive control VLAN or distribution-switch traffic copy, including SCADA/HMI/controller paths when mirrored' },
  ...
  { id: 'tickets-centralstore', source: 'centralstore', target: 'tickets', kind: 'metadata', direction: 'one-way', label: 'approved detections handoff', sourceAnchor: 'left', targetAnchor: 'left', sourcePort: 'metadata', targetPort: 'left', routeClass: 'metadata-handoff', directionCue: 'forward', labelPolicy: 'always', lanePreference: 'external', customerMeaning: 'approved outbound detections and case context hand off into the customer ticket process' },
]
